...
Pace Bar is the standalone package in `PaceBar/`. The root package and its source directories belong to the retained CodexBar project.
...
```sh
make -C PaceBar test check package
```
...
```sh
swift run --package-path PaceBar PaceBar --render-preview /tmp/pace-bar.png
```
