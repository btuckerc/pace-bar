...
Before a draft upload, explicitly create and push the intended `vVERSION` tag at the built commit. Draft/publish require `--confirm btuckerc/usage-bar@vVERSION`; publishing downloads and verifies the draft assets before making them public. The tool never commits, pushes, creates tags, or publishes as a side effect of building. Historical releases and their assets are left unchanged.
