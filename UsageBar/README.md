...
Each bar shows its own estimated exhaustion time (`≈ Sun 3:15 PM`) on the left, with remaining percentage and reset countdown on the right. There is no combined header estimate. `Capped`, `Learning`, and `To reset` distinguish the other states; stale data suppresses estimates. Reserve keeps its own label and bar beneath the main allowance. Exact quota duration, reset, estimate, and calculation basis remain available on hover.

The pace is the mean observed percentage-point consumption per active day in the past 30 **completed local calendar days**. Today is excluded so a partial day's burst does not set the pace. Days with no observed consumption are excluded, and at least two completed active days are required. The tooltip reports the actual number of active days and rate; the app never claims it has 30 days of history before collecting them. Each account, lane, and quota duration has its own history. An unused new cycle can use the account's older history.

Time remaining is **current remaining percentage / historical daily pace × 24 hours**, assuming upcoming days are active. Idle days are not predicted. Predictions stop at that lane's own reset: when reset comes first, the bar says `To reset`. Another account's reset never suppresses the estimate. Shifting work between accounts/models, plan changes, or unusually intense days can change the result.
