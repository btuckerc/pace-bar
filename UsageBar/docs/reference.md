...
The Codex figure follows [T3's usage calculation](https://github.com/pingdotgg/t3code/blob/main/apps/server/src/usage/usagePricing.ts): uncached input × input price + cached input × cache-read price + cache creation × cache-write price + output × output price. Reasoning tokens are already included in output. It is a hypothetical API-price estimate, **not** your subscription bill, savings, quota consumption, or a forecast. Missing models remain unpriced; incomplete history/pricing is labeled **Partial estimate**, and unavailable costs display **—**, never a fabricated zero.
